...
public class LVLServerReplica {

	public static void main(String[] args) {

		try {
			//@formatter:off
			final List<String> replicaList = new ArrayList<>(Arrays.asList(LocationConstants.LAVAL_REPLICA_1,
																		   LocationConstants.LAVAL_REPLICA_2, 
																		   LocationConstants.LAVAL_REPLICA_3));
			//@formatter:on
			
			final ActivityLoggerService activityLogger = new ActivityLoggerService(FileConstants.SERVER_LOG_FILE_PATH + LocationConstants.LAVAL + "/" + args[0].trim()+FileConstants.FILE_TYPE);
	...
	private static void startUdpServer(final ActivityLoggerService activityLogger, final CenterServerImpl server, final List<String> replicaList) {
		...
		try {
			...
			activityLogger.log(MessageTypeConstants.INFO,
					String.format(UdpServerMessages.UDP_SERVER_STARTED, server.getServerName()));
			while (true) {
				try {
